 ...
 *   4. 创建独立 mount namespace, 在其中恢复可见 (logd 自身可见)
 *   5. 保持进程存活, 收到信号时退出并清理
...
static char g_config_path[512] = "/data/local/tmp/logd/config.txt";
...
static void init_empty_dir(void) {
    ...
    if (slash) {
        strcpy(slash + 1, ".logd_empty");
    } else {
        snprintf(g_empty_dir, sizeof(g_empty_dir), "/data/user/0/.logd_empty");
    }
}
...
// 文件控制通道（App 写入命令文件，logd 轮询检测）
static char g_cmd_path[512];

// 初始化控制文件路径（放在 App 私有目录下，App 可直接写入）
static void init_cmd_path(void) {
    strncpy(g_cmd_path, g_config_path, sizeof(g_cmd_path) - 1);
    g_cmd_path[sizeof(g_cmd_path) - 1] = '\0';
    char *slash = strrchr(g_cmd_path, '/');
    if (slash) {
        strcpy(slash + 1, "logd.cmd");
    } else {
        snprintf(g_cmd_path, sizeof(g_cmd_path), "/data/local/tmp/logd/logd.cmd");
    }
}

// 检查控制文件，读取命令
// 返回: 0=无命令, 1=有命令(写入cmd_buf), -1=错误
static int check_cmd_file(char *cmd_buf, int buf_size) {
    int fd = open(g_cmd_path, O_RDONLY);
    if (fd < 0) {
        if (errno == ENOENT) return 0; // 文件不存在，无命令
        return -1;
    }
    
    int n = read(fd, cmd_buf, buf_size - 1);
    close(fd);
...
static void set_process_name(int argc, char **argv, int write_pid)
{
    ...
    if (write_pid) {
        ...
        if (slash) {
            ...
            snprintf(pid_file, sizeof(pid_file), "/data/local/tmp/logd/logd.pid");
        }
...
static int submount_cmp_depth_asc(const void *a, const void *b)
{
    const struct submount_entry *ea = (const struct submount_entry *)a;
    const struct submount_entry *eb = (const struct submount_entry *)b;
    return ea->depth - eb->depth;
}

static int submount_cmp_depth_desc(const void *a, const void *b)
{
    const struct submount_entry *ea = (const struct submount_entry *)a;
    const struct submount_entry *eb = (const struct submount_entry *)b;
...
/*
 * 找出 target 路径下所有子挂载（不包含 target 自身）
 * 返回: 子挂载数量, -1 表示错误
 */
static int find_submounts(const char *target,
                          struct submount_entry *entries, int max_entries)
{
    FILE *f = fopen("/proc/self/mountinfo", "r");
    if (!f) {
        log_print("[子挂载] 无法打开 /proc/self/mountinfo: %s\n", strerror(errno));
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), f) && count < max_entries) {
        int mnt_id, pnt_id;
        char dev[32], root[MAX_MOUNT_PATH], mount_point[MAX_MOUNT_PATH];
        ...
        strncpy(entries[count].mount_point, mount_point, MAX_MOUNT_PATH - 1);
        entries[count].mount_point[MAX_MOUNT_PATH - 1] = '\0';
...
/*
 * 用空目录覆盖目标路径下所有子挂载
 * 从浅到深逐个 bind mount，确保子挂载不会穿透显示
 *
 * 参数:
 *   target    - 目标目录路径
 *   mounted   - 已挂载列表（用于记录和恢复）
 *   mounted_count - 已挂载数量指针
 *   max_mounted   - 已挂载列表最大容量
 *
 * 返回: 成功覆盖的子挂载数量
 */
static int cover_submounts(const char *target,
                           char mounted[][MAX_PKG_LEN],
                           int *mounted_count, int max_mounted)
{
    struct submount_entry entries[MAX_SUBMOUNTS];
    int count = find_submounts(target, entries, MAX_SUBMOUNTS);
    if (count <= 0) return 0;

    log_print("[子挂载] 发现 %d 个子挂载, 正在逐一覆盖...\n", count);

    /* 从浅到深排序，先覆盖上层，再覆盖下层 */
    qsort(entries, count, sizeof(struct submount_entry), submount_cmp_depth_asc);
    ...
    for (int i = 0; i < count; i++) {
        ...
        struct stat mp_st;
        if (stat(mp, &mp_st) != 0) {
            log_print("[子挂载]   跳过: %s (不可访问)\n", mp);
            continue;
        }

        if (!S_ISDIR(mp_st.st_mode)) {
            log_print("[子挂载]   跳过: %s (非目录)\n", mp);
            continue;
        }

        /* 同步空目录属性 */
        sync_empty_attrs(&mp_st);

        /* bind mount 空目录覆盖子挂载 */
        if (mount(g_empty_dir, mp, NULL, MS_BIND, NULL) == 0) {
            log_print("[子挂载]   已覆盖: %s\n", mp);
            if (*mounted_count < max_mounted) {
                snprintf(mounted[*mounted_count], MAX_PKG_LEN, "%s", mp);
                (*mounted_count)++;
            }
            covered++;
        } else {
            log_print("[子挂载]   覆盖失败: %s (errno=%d: %s)\n",
                      mp, errno, strerror(errno));
        }
    }

    if (covered > 0)
        log_print("[子挂载] 成功覆盖 %d/%d 个子挂载\n", covered, count);
    else
        log_print("[子挂载] 未能覆盖任何子挂载\n");
...
static void hide_directories(void)
{
    ...
    for (int i = 0; i < g_hide_dir_count && g_running; i++) {
        ...
        if (!path_exists && (is_android_data_path(path) || is_android_obb_path(path))) {
            ...
            if (stat(real_path, &real_st) == 0) {
                ...
                if (stat(g_empty_dir, &empty_st) == 0 && S_ISDIR(empty_st.st_mode)) {
                    ...
                    cover_submounts(real_path, g_mounted_dirs, &g_mounted_count, MAX_HIDE_DIRS * 2);
        ...
        if (is_android_data_path(path) || is_android_obb_path(path)) {
            ...
            if (stat(g_empty_dir, &empty_st) == 0 && S_ISDIR(empty_st.st_mode)) {
                /* FUSE 路径的子挂载也要覆盖 */
                cover_submounts(path, g_mounted_dirs, &g_mounted_count, MAX_HIDE_DIRS * 2);
...
static void unhide_directories(void)
{
    ...
    /* 从后往前卸载（从深到浅）
     * g_mounted_dirs 中先加入的是子挂载（浅→深），最后加入主目录
     * 反向遍历即可实现从深到浅卸载，避免父目录先卸载导致子目录 umount 失败 */
    ...
    log_print("[恢复] 完成: 成功 %d 个, 失败 %d 个\n", restored, failed);
    if (failed > 0) {
        for (int i = g_mounted_count - 1; i >= 0; i--) {
            if (umount2(g_mounted_dirs[i], MNT_DETACH) != 0) {
                log_print("[警告] 恢复失败: %s (errno=%d)\n", g_mounted_dirs[i], errno);
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2 && strcmp(argv[1], "--cleanup") == 0) {
        if (argc >= 3 && argv[2][0])
            ...
            snprintf(g_config_path, sizeof(g_config_path), "%s",
                     "/data/local/tmp/logd/config.txt");
        ...
        for (int i = 0; i < g_hide_dir_count; i++) {
            ...
            /* 先清理子挂载上的 bind mount 残留（从最深到最浅）
             * 守护进程崩溃时，子挂载上的覆盖 bind 可能残留 */
            {
                struct submount_entry entries[MAX_SUBMOUNTS];
                int sub_count = find_submounts(path, entries, MAX_SUBMOUNTS);
                if (sub_count > 0) {
                    log_print("[cleanup] 检测到 %d 个子挂载, 尝试清理...\n", sub_count);
                    qsort(entries, sub_count, sizeof(struct submount_entry),
                          submount_cmp_depth_desc);
                    for (int s = 0; s < sub_count; s++) {
                        const char *sub = entries[s].mount_point;
                        for (int attempt = 0; attempt < 3; attempt++) {
                            if (umount2(sub, MNT_DETACH) == 0) {
                                log_print("[cleanup] umount sub: %s\n", sub);
                                umounted = 1;
                            } else {
                                break;
                            }
    ...
    if (hide_list_has_storage_paths()) {
         ...
         * 5. 在 logd namespace 中 umount bind (恢复, logd 恢复可见)
         * 6. logd namespace 中所有路径恢复正常 (无需额外 bind)
         */
        ...
        for (int i = 0; i < g_hide_dir_count; i++) {
            ...
            if (sub_count > 0) {
                qsort(entries, sub_count, sizeof(struct submount_entry),
                      submount_cmp_depth_desc);
                for (int s = 0; s < sub_count; s++) {
                    umount2(entries[s].mount_point, MNT_DETACH);
                    umount2(entries[s].mount_point, MNT_DETACH);
                }
            }
        ...
        if (read_ns("/proc/self/ns/mnt", logd_ns, sizeof(logd_ns)) > 0) {
            log_print("[命名空间] logd namespace: %s\n", logd_ns);
        }
        ...
        /* 6. 在 logd namespace 中恢复 (umount bind mounts)
         * 从后往前卸载（从深到浅），避免父目录先卸载导致子目录 umount 失败 */
        log_print("[恢复] 在 logd namespace 中恢复目录可见性\n");
        for (int i = g_mounted_count - 1; i >= 0; i--) {
            if (umount2(g_mounted_dirs[i], MNT_DETACH) == 0) {
                log_print("[恢复] logd 可见: %s\n", g_mounted_dirs[i]);
            }
        }

    } else {
        /* === 普通目录模式: 进入 init namespace → bind mount → 创建独立 namespace → 自身恢复 ===
         * su 启动时在 app namespace, 需要进入 init namespace
         * 才能让 bind mount 对所有进程生效
         * 然后创建独立 namespace 让 logd 自己仍然能看到真实内容
         */
        log_print("[隔离] 模式: 普通目录 bind mount (带独立 namespace)\n");
    ...
    while (g_running) {
        ...
        int ret = check_cmd_file(cmd_buf, sizeof(cmd_buf));
        if (ret <= 0) continue; // 无命令或出错，继续

        log_print("[控制] 收到命令: %s\n", cmd_buf);

        if (strcmp(cmd_buf, "unhide") == 0 || strcmp(cmd_buf, "stop") == 0 || strcmp(cmd_buf, "exit") == 0) {
            log_print("[控制] 收到退出命令，开始恢复...\n");
            g_running = 0;
            break;
    ...
    /* 退出清理：先回到 init namespace 再 umount，才能真正清理 init namespace 中的 bind mount */
    log_print("[退出] logd 正在停止\n");
    log_print("[退出] 进入 init namespace 进行清理...\n");
    if (enter_init_namespace() == 0) {
        log_print("[退出] 已进入 init namespace，开始恢复...\n");
    } else {
        log_print("[退出] 警告: 无法进入 init namespace，尝试在当前 namespace 恢复\n");
    }
    unhide_directories();
    cleanup_pid_file();
    log_print("[退出] logd 已停止\n");
    return 0;
}
