/*
 * guard_storage.c - Android 16 FUSE Storage Mount Namespace 隔离方案
 *
 * 完整实现规范:
...
static void log_print(const char *fmt, ...)
{
    ...
    if (clock_gettime(CLOCK_REALTIME, &ts) == 0) {
        ...
        fprintf(stderr, "[%s.%03ld] [Guard] ", time_buf, ts.tv_nsec / 1000000);
    }
...
static int detach_storage_fuse(void)
{
    ...
    {
        const char *temp_base = "/data/local/tmp/.guard_storage";
...
int main(int argc, char *argv[])
{
    ...
    if (read_ns("/proc/self/ns/mnt", guard_ns, sizeof(guard_ns)) < 0) {
        log_print("ERROR: 无法读取 guard namespace\n");
        return 1;
    }
    log_print("Guard mount namespace: %s\n", guard_ns);

    /* --- 验证 namespace 确实不同 --- */
    if (strcmp(host_ns, guard_ns) == 0) {
        log_print("ERROR: namespace 未改变!\n");
        return 1;
    }
    log_print("Namespace 隔离: YES (宿主=%s, guard=%s)\n", host_ns, guard_ns);
